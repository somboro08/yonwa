...
class _BookingConfirmationScreenState extends State<BookingConfirmationScreen> {
        ...
        return Theme(
          data: Theme.of(context).copyWith(
            colorScheme: ColorScheme.light(
              ...
              onSurface: YonwaColors.neutral800,
            ),
          ),
          child: child!,
        );
  ...
  void _showSuccessDialog() {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(YonwaRadius.lg)),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            const Icon(Icons.check_circle_rounded, color: YonwaColors.success, size: 64),
            const SizedBox(height: YonwaSpacing.md),
            const Text('Demande envoyée !', style: YonwaTextStyles.h2),
            ...
            SizedBox(
              ...
              child: OutlinedButton(
                onPressed: () async {
                  ...
                  await Printing.sharePdf(bytes: pdfBytes, filename: 'recu_yonwa_reservation.pdf');
                },
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Confirmer la réservation', style: YonwaTextStyles.h3),
        centerTitle: true,
      ),
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
              ...
              child: Row(
                children: [
                  Container(
                    ...
                    child: const Icon(Icons.home_rounded, color: YonwaColors.primary500),
                  ),
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        Text('${widget.listing.quartier}, ${widget.listing.ville}', style: YonwaTextStyles.caption.copyWith(color: YonwaColors.neutral500)),
                        const SizedBox(height: 4),
                        Text('${widget.listing.prixParNuit.toInt()} FCFA / nuit', style: YonwaTextStyles.label.copyWith(color: YonwaColors.primary500)),
                      ],
                    ),
                  ),
                ],
            ...
            // Date Selection
            Text('Dates du séjour', style: YonwaTextStyles.h3),
            const SizedBox(height: YonwaSpacing.md),
            GestureDetector(
              ...
              child: Container(
                ...
                child: Row(
                  children: [
                    const Icon(Icons.calendar_today_rounded, size: 20, color: YonwaColors.primary500),
                    const SizedBox(width: YonwaSpacing.md),
                    Text(
                      _startDate == null 
                        ? 'Sélectionner les dates' 
                        : '${dateFormat.format(_startDate!)} — ${dateFormat.format(_endDate!)}',
                      style: YonwaTextStyles.label,
                    ),
                    const Spacer(),
                    const Icon(Icons.edit_rounded, size: 16, color: YonwaColors.neutral400),
                  ],
                ),
              ),
            ),
            ...
            // Guests Selection
            Text('Voyageurs', style: YonwaTextStyles.h3),
            const SizedBox(height: YonwaSpacing.md),
            Container(
              padding: const EdgeInsets.symmetric(horizontal: YonwaSpacing.md, vertical: 8),
              decoration: BoxDecoration(
                color: isDark ? YonwaColors.neutral800 : Colors.white,
                borderRadius: BorderRadius.circular(YonwaRadius.md),
                border: Border.all(color: isDark ? YonwaColors.neutral700 : YonwaColors.neutral200),
              ...
              child: Row(
                children: [
                  const Icon(Icons.people_outline_rounded, size: 20, color: YonwaColors.primary500),
                  const SizedBox(width: YonwaSpacing.md),
                  Text('$_guests personne${_guests > 1 ? 's' : ''}', style: YonwaTextStyles.label),
                  const Spacer(),
                  IconButton(
            ...
            // Price Summary
            Text('Détails du prix', style: YonwaTextStyles.h3),
            const SizedBox(height: YonwaSpacing.md),
            _SummaryRow(
            ...
            _SummaryRow(
              label: 'Frais de service Yonwa',
              value: '500 FCFA',
              isDark: isDark,
            ),
            const Divider(height: YonwaSpacing.lg),
            _SummaryRow(
              label: 'Total',
            ...
            const SizedBox(height: YonwaSpacing.xxl),
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                ...
                child: _isProcessing
                    ...
                    : const Text('Confirmer la réservation', style: YonwaTextStyles.button),
              ),
            ),
            const SizedBox(height: YonwaSpacing.xl),
          ],
      ...
      child: Row(
        ...
        children: [
          Text(
            label,
            style: isTotal 
              ? YonwaTextStyles.label.copyWith(fontWeight: FontWeight.bold, fontSize: 16)
              : YonwaTextStyles.body.copyWith(color: YonwaColors.neutral500),
          ),
          Text(
            value,
            style: isTotal
              ? YonwaTextStyles.h3.copyWith(color: YonwaColors.primary500, fontWeight: FontWeight.bold)
              : YonwaTextStyles.label.copyWith(color: isDark ? YonwaColors.neutral200 : YonwaColors.neutral700),
          ),
        ],
      ),
    );
